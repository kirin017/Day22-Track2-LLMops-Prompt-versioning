...
class JSONFormatter(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        try:
            ...
            return FailResult(error_message="Repaired JSON", fix_value=repaired)
        except json.JSONDecodeError as e:
            return FailResult(
                error_message=f"Invalid JSON after repair attempt: {e}",
                fix_value=json.dumps({"error": f"Invalid JSON after repair attempt: {e}", "raw": value})
            )
...
def demo_json_guard():
    ...
    for label, text in test_cases:
        result = guard.validate(text)
        status = "✅ Pass/Fixed" if result.validation_passed or result.validated_output else "❌ Fail"
        print(f"\n[{label}] {status}")
